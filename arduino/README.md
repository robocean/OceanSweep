...
위 식을 이용하여 직전 추정값인 $x_{k-1}$을 다음과 같이 정의할 수 있다.

$x_{k-1} = \alpha x_{k-2} + (1 - \alpha) z_{k-1}$

이 식을 $x_k$에 대입해 보면 다음과 같다.

$x_k = \alpha (\alpha x_{k-2} + (1 - \alpha) z_{k-1}) + (1 - \alpha) z_k$\
$x_k=\alpha^2x_{k-2} + \alpha(1 - \alpha) z_{k-1} + (1 - \alpha) z_k$

이때 $\alpha$는 $0<\alpha<1$이므로, 다음과 같은 관계가 성립한다.\
$\alpha^2 <\alpha(1 - \alpha) <(1 - \alpha)$


여기서 가중치 $\alpha$를 조절하여 필터의 특성을 변경할 수 있다. $\alpha$를 낮게 설정하여 현재 측정값($z_k$)에 더 큰 가중치($1 - \alpha$)를 부여하면, 추정값이 현재 측정값을 많이 반영하여 상태변화를 빠르게 나타내지만, 노이즈에 민감해 진다. 반대로 $\alpha$를 높게 설정하여 현재 측정값에 낮은 가중치를 부여하면, 노이즈 억제 성능이 향상된다.

이 필터는 이전 추정값($x_{k-1}$)만을 저장하면 되므로, 일반적인 이동평균필터에 비해 메모리 사용 측면에서 이점이 있다. 
또한, $\alpha$ 값을 적절히 조절함으로써 추정값이 현재 상태를 빠르게 반영할지, 노이즈억제 성능을 높일지 선택할 수 있다는 장점이 있다.

### Sawtooth 선형화 알고리즘

IMU의 방위각 데이터가 0도와 360도 사이를 반복하며 발생하는 Sawtooth 현상은 제어 알고리즘의 오작동을 유발할 수 있다. 예를 들어, 현재 방위각 350도에서 반시계방향으로 20도 회전하면 로봇의 방위각은 10도가 된다. 그러나 데이터상에서는 시계방향으로 340도 회전한것처럼 보인다. 이러한 문제를 해결하기 위해 **연속적인 값으로 변환하는 선형화 알고리즘**을 적용한다. 이 알고리즘은 각도가 360도를 초과하거나 0도 미만으로 떨어질 때 적절한 보정을 수행하여 항상 연속적인 값을 유지하도록 한다.
